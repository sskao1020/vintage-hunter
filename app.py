...
def scrape_yahoo_auction(keyword):
    """【日本雅虎拍賣】爬蟲"""
    url = f"https://auctions.yahoo.co.jp/search/search?p={keyword}&va={keyword}&is_postage_mode=1&dest_pref_code=13&exflg=1&b=1&n=30&s1=new&o1=d"
    headers = {
...
def scrape_carousell_taiwan(keyword):
    """【台灣旋轉拍賣】爬蟲"""
    url = f"https://tw.carousell.com/search/{keyword}/?sort_by=time_created%2Cdescending"
    headers = {
...
def scrape_shopee_taiwan(keyword):
    ...
    url = f"https://shopee.tw/api/v4/search/search_items?keyword={keyword}&limit=30&newest=0&by=ctime&order=desc"
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
